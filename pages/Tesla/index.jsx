...
export const getServerSideProps = async () => {
  if (!process.env.NODE_ENV || process.env.NODE_ENV === "development") {
    ...
    const apiResponse = await fetch(
      `https://newsapi.org/v2/everything?q=tesla&sortBy=publishedAt&apiKey=${process.env.URL_KEY}`
    );
    ...
    const apiResponse = await fetch(
      `https://newsapi.org/v2/everything?q=tesla&sortBy=publishedAt&apiKey=35c3878043074acdbc44b69e156687db`
    );
    const res = await apiResponse.json();
    const { articles } = res;
    return {
